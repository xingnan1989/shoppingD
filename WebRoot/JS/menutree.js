var faisunMenu_openedfolderimage_src="images/jian_null.gif";
var faisunMenu_closedfolderimage_src="images/jia.gif";
var faisunMenu_menufileimage_src="images/jian.gif";
...
function addtree(text,url,target){
  ...
  faisunMenu_treeurltarget.push(target?target:"main");
}
...
function createtree(){
  ...
  for(i=0;i<faisunMenu_treetext.length;i++){
	...
	for(j=1;j<subnum;j++){
	  echo += "<td width='25' valign=bottom><img src='images/"+(openedlayer[j]?"bar3.gif":"spacer.gif")+"' ></td>\n";
	}
	if(subnum>0){
	  echo += "<td width='25' valign=bottom><img src='images/bar"+barstatus+".gif' ></td>";
	}
	var clicktoshowhide=(havechild?" onclick='showhideit("+faisunMenu_treeNum+""+i+")' style='cursor:pointer;' ":"");
    echo += "<td width='25' valign=bottom "+clicktoshowhide+"><img name='openedfolderimage"+faisunMenu_treeNum+""+i+"' src='"+(havechild?faisunMenu_openedfolderimage_src:faisunMenu_menufileimage_src)+"' class='"+(openfold?"showme":"hideme")+"' ><img name='closedfolderimage"+faisunMenu_treeNum+""+i+"' src='"+(havechild?faisunMenu_closedfolderimage_src:faisunMenu_menufileimage_src)+"' class='"+(openfold?"hideme":"showme")+"' ></td>";
